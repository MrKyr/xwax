...
int osc_send_load_track(char *server, int d, char *path)
{
    lo_address t = lo_address_new(server, "7770");
    
    if (lo_send(t, "/xwax/load_track", "isssd", d, path, "Artist", "Title", 0.0) == -1) {
        fprintf(stderr, "OSC error %d: %s\n", lo_address_errno(t),
            lo_address_errstr(t));
        return 1;
    }
...
{
    fprintf(fd, "Usage: xwax-client <server> load <deck-number> "
...
int main(int argc, char *argv[])
{
    ...
    if(strcmp(argv[2], "load") == 0) {
        ...
        return osc_send_load_track(server, d, path);
